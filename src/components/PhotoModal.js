    ...
    if (data.length > 0) {
        return data.map(
            item => {
                ...
                return (
                    <TouchableOpacity onPress={()=> onSelectItem(item.node.image.uri) } style={{ width: SCREEN_WIDTH, height: SCREEN_WIDTH }}>
...
const PhotoModal = ({ showModal, onCloseModal, data,onSelectItem, selectedImages}) => {
    return (
            ...
            <View style={{ flex: 1, justifyContent: 'flex-end' }} >
                <View style={{ width: '100%', height: '85%', backgroundColor: 'black' }} >
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between' }} >
                        ...
                        <Button type="clear" title="Agregar" titleStyle={{ color: config.colors.green, fontSize: 20 }} />
                    </View>
